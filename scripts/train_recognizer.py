...
def load_partition(processed: Path, users, cap: int, seed: int):
    x_blocks, y_blocks = [], []
    ...
    for uuid in users:
        ...
        x_blocks.append(extract_features(windows[selected]))
...
def main() -> int:
    ...
    x_train, y_train, classes = load_partition(processed, splits["train"], cap, seed)
    x_val, y_val, _ = load_partition(processed, splits["validation"], cap, seed + 1)
    ...
    x_test, y_test, _ = load_partition(processed, splits["recognition_test"], cap, seed + 2)
